          ...
          <div className="grid gap-2">
            ...
            <div className="text-gray-500 dark:text-gray-400">
              {session?.user?.roles?.map((role) => (
                <span
                  key={role}
                  className="mr-2 rounded-full bg-blue-100 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
                >
                  {role}
                </span>
              ))}
          ...
          <div className="grid gap-2">
            ...
            <div className="text-gray-500 dark:text-gray-400">
              {session?.user?.permissions?.map((permission) => (
                <span
                  key={permission}
                  className="mr-2 rounded-full bg-green-100 px-2 py-1 text-xs font-medium text-green-800 dark:bg-green-900/30 dark:text-green-300"
                >
                  {permission}
                </span>
              ))}
